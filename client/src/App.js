...
function App() {
  ...
  const getDetails = (e) => {
    ...
    .then(res => {
      if (res.ok) {console.log("successful get");}
          ...
          movie.map((film, index) => (
            <div key={index}>
              <Film
                ...
                type={film.Type} 
              />
              <button type="button" className="favButton" onClick={handleFavourites}>Add to favourites</button>
              <button type="button" onClick={getDetails} data-id={film.imdbID}>Details</button>
